...
object Json4sJacksonNoTypeHintsProtocol extends Json4sJacksonSupport {
  ...
  var customSerializers : Seq[Serializer[_]] = Seq.empty

  def registerSerializer(serializer : Serializer[_]) = {
    customSerializers = customSerializers :+ serializer
  }
  override implicit def json4sJacksonFormats: Formats = DefaultFormats.withHints(NoTypeHints) ++ customSerializers
}
